 ...
 * @returns
 */
function longestCommonSubsequenceWithMemoization(strings: string[], numbers: number[], memoArray: number[][]): number {
	const s1 = strings[0];
	const s2 = strings[1];
	const m = numbers[0];
	const n = numbers[1];
	...
	if (s1.charAt(m - 1) === s2.charAt(n - 1)) {
		...
		memoArray[m][n] = Math.max(longestCommonSubsequenceWithMemoization([s1, s2], [m, n - 1], memoArray), longestCommonSubsequenceWithMemoization([s1, s2], [m - 1, n], memoArray));
	}
...
function longestCommonSubsequenceWrapper(s1: string, s2: string) {
	...
	const memoArray: number[][] = Array.from({length: m + 1}, () => Array.from({length: n + 1}, () => 0));
